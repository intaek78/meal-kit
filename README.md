...
4. 판매자가 상품을 등록한다.
6. 회원이 상품을 선택하여 주문한다.
7. 고객이 결제하고, 결제완료되면 판매자에게 알림을 보낸다.
8. 결제완료되면 재고수량 변경을 한다.
8. 판매자가 확인하여 상품을 배송한다.
9. 배송이 완료되면 고객이 완료 처리한다.
9. 고객이 주문 취소할 수 있다.
10. 주문취소되면 배송이 취소된다.
11. 주문취소되면 결제가 취소된다.
12. 고객이 주문내역(상태)를 중간중간 조회가능하다.


비기능적 요구사항
1. 트랜잭션
    1. 결제가 되지 않은 주문건은 아예 거래가 성립되지 않아야 한다  Sync 호출
    2. 주문취소는 배송이 시작되지 않은 건만 가능하다. Sync 호출
    3. 주문취소는 결제가 취소되어야만 가능하다. Sync 호출
1. 장애격리
    1. 상품관리 기능이 수행되지 않더라도 주문은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    1. 결제시스템이 과중되면 사용자를 잠시동안 받지 않고 결제를 잠시후에 하도록 유도한다  Circuit breaker, fallback
1. 성능
    1. 고객이 자주 배달상태를 마이페이지에서 확인할 수 있어야 한다  CQRS
    1. 결제까지 완료된 구매건에 대해서 판매자에게 알림을 줄 수 있어야 한다  Event driven
    1. 배송상태가 변경되면 고객에게 알림을 줄 수 있어야 한다.
